...
export class LessonPlanService {
  ...
  private generatePeriods(settings: any, summary: any) {
    ...
    let currLPStorage: any = this._storageService.getItemSession(`my_lessonPlan`)
    if (currLPStorage) {
      const isLPExist: boolean = currLPStorage.some((elem: any) => elem.info.id === summary.id)
      if (!isLPExist) {
        currLPStorage.push(periods)
        this._storageService.setItemSession(`my_lessonPlan`, currLPStorage)
      } else {
        currLPStorage = currLPStorage.filter((elem: any) => elem.info.id !== summary.id)
        currLPStorage.push(periods)
        this._storageService.setItemSession(`my_lessonPlan`, currLPStorage)
      }
    } else {
  ...
  private associateSubTitlesPeriods(periods: any, subtitles: any) {
    ...
    periods.data.map((period: any) => {
      isholding = 0
      if (tmpSubtitle) {
        if (tmpSubtitle.duration > period.duration || (tmpSubtitle.duration + isholding) > period.duration) {
