...
@Component({
  selector: 'app-madquestion-mcqoption',
  templateUrl: './madquestion-mcqoption.component.html',
  styleUrls: ['./madquestion-mcqoption.component.css'],
})
export class MADQuestionMCQoptionComponent implements OnInit {
  isLastElement = false;
  @Input() answer: any;
  content = '';
  @Input() editorisToggled = false;
  @Input() ReferenceCollapser;
  @Input() referenceArray: BehaviorSubject<any>;
  ...
  didAdd() {
    const letArr: any[] = this.referenceArray.getValue();
    letArr.push({
      answerText: '',
      isCorrect: false,
    });
    this.referenceArray.next(letArr);
  }
}
