...
import { Class, LearningStandard, Subject, Type, Unit } from 'src/app/modules/Models/curriculum';
...
export class CurriculumComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.isDefaultConf$.subscribe(
      status => this.defaultConf = status
    )
  }
  ...
  filter() {
    if (this.selectedClass.id > 0 && this.selectedSubject.id > 0) {
      this._curriculumService.getCurriculum(this.selectedClass, this.selectedSubject)
      this._curriculumService.getLearningStandards(this.selectedClass.id, this.selectedSubject.id)
  ...
  applyDefaultConf() {
    if (this.applyDefConf.checked) {
      ...
      this._confirmDialogService.confirmed().subscribe(confirmed => {
        if (confirmed) {
          this._curriculumService.applyDefaultConf(this.selectedClass.id, this.selectedSubject.id, this.curriculumCurrType, this.applyDefConf.checked)
  ...
  onAddUnit(){
    ...
    this._curriculumService.addUnit(this.defaultApplied, this.selectedClass, this.selectedSubject, this.selectedCurriculumType, this.addUnitForm.value.code, this.addUnitForm.value.title)
    this.addUnitForm.reset()
  ...
  onAddLearningStandard(){
    ...
    this._curriculumService.addLearningStandard(this.defaultApplied, this.selectedClass, this.selectedSubject, this.parentUnitId, this.addLearningStandardForm.value.code, this.addLearningStandardForm.value.description, this.addLearningStandardForm.value.relatedTo)
    this.addLearningStandardForm.reset()
    this.matDrawer.close()
  }
  ...
  onUnitUpdate(isDefault: boolean, unitId: number){
    ...
    this._curriculumService.updateUnit(isDefault, unitId, this.selectedClass, this.selectedSubject, updatedUnitArray[0], updatedUnitArray[1])
  ...
  onLsUpdate(isDefault: boolean, lsId: number){
    ...
    this._curriculumService.updateLearningStandard(isDefault, lsId, this.selectedClass, this.selectedSubject, updatedLsArray[0], updatedLsArray[1])
  ...
  deleteUnit(isDefault: boolean, unitId: number){
    ...
    this._confirmDialogService.confirmed().subscribe(confirmed => {
      if (confirmed) {
        this._curriculumService.deleteUnit(isDefault, unitId, this.selectedClass, this.selectedSubject)
      }
    })
  }
  ...
  deleteLearningStandard(isDefault: boolean, learningStandardId: number){
    ...
    this._confirmDialogService.confirmed().subscribe(confirmed => {
      if (confirmed) {
        this._curriculumService.deleteLearningStandard(isDefault, learningStandardId, this.selectedClass, this.selectedSubject)
      }
    })
  }

  toggleAllSelection() {
    this._funcsService.toggleAllSelection(this.allSelected, this.addLearningStandardForm.controls['relatedTo'], this.learningStandards)
  }
}
